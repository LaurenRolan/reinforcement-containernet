...
linkopts_switches = dict(bw=11, delay='1ms', loss=0, max_queue_size=1000, use_htb=True)
linkopts_hosts = dict(bw=5, delay='1ms', loss=0, max_queue_size=1000, use_htb=True)
hosts = []
                ...
                for k, v in route.items():
                    if v[1] == Traffic_Switches.HIGH: # High traffic
                        for alternative in self.paths[k]:
                            if self.default_route[k] != alternative and (k[0] in self.list_servers or k[1] in self.list_servers):
            ...
            try:
                ...
                try:
                    ...
                    if filename == "paths": # Receives only one time
                        ...
                        for k,v in self.paths.items(): # set default
                        ...
                        if self.currentProfile == Profile.BALANCE:
                            ...
                            if timestep < 60:
                                ...
                                self.action = self.pick_action_traffic(state_index=current_state_id, epsilon=0.1)
...
call("cd ~/containernet/bin/ && sudo sh clear_crash.sh", shell=True) # sudo netstat -tulpn
